...
std::pair<Status, Data> StorageBackend::read_chunk(const std::string &chunk_id) {
    ...
    if (fstat(fd, &st) < 0) {
        ::close(fd);
        return { Status::IOError("fstat failed: " + path + " (" + strerror(errno) + ")"), result };
    }
    ...
    if (!result.ParseFromString(buf)) {
        return { Status::IOError("Failed to parse Data protobuf from: " + path), result };
    }

    printBytesAsHex(
        reinterpret_cast<const unsigned char*>(result.payload().data()), 
        result.len()
    );
    return { Status::OK(), result };
}

std::pair<Status, size_t> StorageBackend::write_chunk(const std::string &chunk_id, const Data &data) {
    std::string path = get_chunk_path(chunk_id);
    ...
    ssize_t bytes_written = ::pwrite(fd, serialized.data(), serialized.size(), 0);
    ::close(fd);
