...
static int torchfs_readdir(const char* path, void* buf, fuse_fill_dir_t filler,
    ...
    // Make sure that TorchFuse::readdir is updated to either ignore the flags parameter
    // or you adapt it to accept the extra parameter.
    return fs->readdir(path, buf, filler, offset, fi);
}

// Updated wrapper for lseek: now returns off_t.
static off_t torchfs_lseek(const char* path, off_t offset, int whence, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
    return fs->lseek(path, offset, whence, fi);
}

// Other wrappers remain the same
static int torchfs_getattr(const char* path, struct stat* stbuf, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
...
static int torchfs_rename(const char* from, const char* to, unsigned int flags) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
...
static int torchfs_chmod(const char* path, mode_t mode, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
    return fs->chmod(path, mode, fi);
}

static int torchfs_chown(const char* path, uid_t uid, gid_t gid, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
    return fs->chown(path, uid, gid, fi);
}

static int torchfs_truncate(const char* path, off_t size, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
    return fs->truncate(path, size, fi);
}

static int torchfs_create(const char* path, mode_t mode, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
...
static int torchfs_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
    return fs->read(path, buf, size, offset, fi);
}

static int torchfs_write(const char* path, const char* buf, size_t size, off_t offset, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
...
static int torchfs_fsync(const char* path, int isdatasync, struct fuse_file_info* fi) {
    TorchFuse* fs = static_cast<TorchFuse*>(fuse_get_context()->private_data);
