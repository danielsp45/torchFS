...
TorchFuse::TorchFuse(const std::string& root)
    : root_(root)
{
...
int TorchFuse::getattr(const char* path, struct stat* stbuf, struct fuse_file_info *fi) {
	(void) fi;
	int res;
...
int TorchFuse::chown(const char* path, uid_t uid, gid_t gid, struct fuse_file_info* fi) {
	(void) fi;
...
int TorchFuse::truncate(const char* path, off_t size, struct fuse_file_info* fi) {
    int res;
    if (fi != NULL)
...
int TorchFuse::create(const char* path, mode_t mode, struct fuse_file_info* fi) {
...
int TorchFuse::read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi) {
	int fd;
	int res;
...
int TorchFuse::write(const char* path, const char* buf, size_t size, off_t offset, struct fuse_file_info* fi) {
    int fd;
    int res;
    if (fi == NULL)
...
int TorchFuse::fsync(const char* path, int isdatasync, struct fuse_file_info* fi) {
    (void) path;
    if (isdatasync == 0) {
        return ::fsync(fi->fh);
    }
...
int TorchFuse::lseek(const char* path, off_t offset, int whence, struct fuse_file_info* fi) {
    int fd;
    off_t res;
    if (fi == NULL)
