...
static int xmp_unlink(const char *path)
{
	...
	Indexmeta* meta;
	if (index_get(index, path, &meta) == 0) {
		meta->deleted = 1;
	} else {
		      ...
		      struct fuse_file_info *fi)
{
	...
	// if the file name is END, flush the index to the disk	
	if (strcmp(path, "/backend/FLUSH") == 0) {
		index_flush(index, "/tmp/index_logs/index.log");
		pthread_mutex_unlock(&p_ctx->mutex);
		return 0;
	} else if (strcmp(path, "/backend/RESET") == 0) {
		index_destroy(index);
		p_ctx->index = index_init();
		pthread_mutex_unlock(&p_ctx->mutex);
		return 0;
	}

	Indexmeta* meta = malloc(sizeof(Indexmeta));
	meta->size = 0;
	meta->num_access = 0;
	meta->num_metadata = 0;
	meta->num_lseek = 0;
...
static int xmp_open(const char *path, struct fuse_file_info *fi)
{
    ...
    Indexmeta* meta;
    if (index_get(p_ctx->index, path, &meta) == 0) {
        // Update the metadata
	    meta->num_metadata++;
    } else {
        printf("File not found in index\n");
		    ...
		    struct fuse_file_info *fi)
{
	...
	if (index_get(index, path, &meta) == 0) {
		...
		printf("File not found in index\n");
		// create and entry in the index with default values
		meta = malloc(sizeof(Indexmeta));
		meta->size = 0;
		meta->num_access = 1;
		meta->num_metadata = 0;
		meta->num_lseek = 0;
...
static int xmp_statfs(const char *path, struct statvfs *stbuf)
{
	...
	printf("[Thread %d] Statfs for path %s, userid %d, pid %d\n", gettid(), path, f_ctx->uid, f_ctx->pid);
	fprintf(p_ctx->fp,"[Thread %d] Statfs for path %s, userid %d, pid %d\n", gettid(), path, f_ctx->uid, f_ctx->pid);
	pthread_mutex_unlock(&p_ctx->mutex);
#endif
