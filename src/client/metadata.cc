...
std::vector<Dirent> MetadataStorage::readdir(const uint64_t& inode) {
    ...
    std::unique_ptr<rocksdb::Iterator> it(db_->NewIterator(read_options, cf_dentry_));
...
FileInfo MetadataStorage::open(const uint64_t& inode) {
    ...
    rocksdb::Status status = db_->Get(read_options, cf_inode_, key, &value);
    if (!status.ok()) {
        throw std::runtime_error("Failed to get file info: " + status.ToString());
    }
...
Status MetadataStorage::create_file(const uint64_t &p_inode, const std::string &name) {
    Attributes attr;
    ...
    if (!status.ok()) {
        return Status::IOError("Failed to create directory entry: " + status.ToString());
    }
...
Status MetadataStorage::create_dir(const uint64_t &p_inode, const std::string &name) {
    Attributes attr;
    attr.set_inode(next_inode_++);
    attr.set_path(name);
    attr.set_creation_time(time(nullptr));
    ...
    rocksdb::Status status = db_->Put(write_options, cf_inode_, key, value);
    if (!status.ok()) {
        return Status::IOError("Failed to create directory: " + status.ToString());
    }
...
Status MetadataStorage::remove_file(const uint64_t &p_inode, const uint64_t &inode) {
    ...
    std::string dentry_key = std::to_string(p_inode) + ":" + std::to_string(inode);
...
Status MetadataStorage::remove_dir(const uint64_t &inode) {
    ...
    rocksdb::Status status = db_->Delete(write_options, cf_inode_, key);
    if (!status.ok()) {
        return Status::IOError("Failed to remove directory: " + status.ToString());
    }

    // INFO: Since every file is removed before removing the directory, we don't need to remove the dentry entry

    return Status::OK();
}

Status MetadataStorage::rename_file(const uint64_t &old_p_inode, 
    const uint64_t &new_p_inode, const uint64_t &inode, const std::string &new_name) {
        ...
        std::string old_dentry_key = std::to_string(old_p_inode) + ":" + std::to_string(inode);
        status = db_->Delete(write_options, cf_dentry_, old_dentry_key);
        if (!status.ok()) {
            return Status::IOError("Failed to remove old directory entry: " + status.ToString());
        }
        ...
        if (!status.ok()) {
            return Status::IOError("Failed to create new directory entry: " + status.ToString());
        }
        return Status::OK();
}

Status MetadataStorage::rename_dir(const uint64_t &old_p_inode, const uint64_t &new_p_inode, const uint64_t &inode, const std::string &new_name) {
    // go to inode entry and change the name of the file
    Attributes attr = getattr(inode);
    attr.set_path(new_name);
    std::string value;
    ...
    rocksdb::Status status = db_->Put(write_options, cf_inode_, key, value);
    if (!status.ok()) {
        return Status::IOError("Failed to rename directory: " + status.ToString());
    }
